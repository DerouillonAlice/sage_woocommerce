...
<div class="mb-4 p-4 rounded-lg border-l-4 shadow-sm" role="alert" style="border-left-color: var(--color-error); background-color: color-mix(in srgb, var(--color-error) 10%, white);" <?php echo $multiple ? '' : wc_get_notice_data_attr( $notices[0] ); ?>>
		...
		<div class="ml-3 flex-1">
			<?php if ( $multiple ) { ?>
				<div class="font-medium mb-2" style="color: color-mix(in srgb, var(--color-error) 80%, black);">
					<?php esc_html_e( 'Les problèmes suivants ont été trouvés :', 'woocommerce' ); ?>
				</div>
