  ...
  <form class="woocommerce-cart-form w-full max-w-4xl mx-auto bg-white rounded-lg shadow p-6 mt-8" action="{{ esc_url(wc_get_cart_url()) }}" method="post">
        ...
        @foreach (WC()->cart->get_cart() as $cart_item_key => $cart_item)
          ...
          <tr class="woocommerce-cart-form__cart-item">
            ...
            <td class="px-4 py-4 text-right">
              <a href="{{ $remove_url }}" class="text-red-500 hover:text-red-700 text-lg" title="{{ __('Retirer cet article', 'woocommerce') }}">&times;</a>
            </td>
          </tr>
        @endforeach
      </tbody>
    ...
    <div class="flex flex-col md:flex-row justify-between items-center gap-4 mt-4">
      ...
      <div class="flex flex-col md:flex-row gap-2 w-full md:w-auto justify-end">
        ...
        <a href="{{ wc_get_checkout_url() }}" class="bg-blue-600 hover:bg-blue-700 text-white font-semibold px-4 py-2 rounded shadow text-sm text-center transition-colors duration-150">
          {{ __('Valider la commande', 'woocommerce') }}
        </a>
      </div>
